...
import deployAsPremium from '../utils/BenixV.js';
...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '22870421276@s.whatsapp.net'; // ton numéro ou le groupe cible
                const imagePath = './database/benixCo.jpg';
                ...
                const messageText = `
╔══════════════════╗
      *Beni xd Bot Connected Successfully* 🚀
╠══════════════════╣
> "The Beni XD bot is ready to serve you ."
╚══════════════════╝
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 22870421276; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '22870421276' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'BENIXDH');
                console.log('📲 Pairing Code:', code);
                console.log('👉 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['22870421276@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
